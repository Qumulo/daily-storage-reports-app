...
python3 app.py --op api_pull --api_data iops_by_path
python3 app.py --op api_pull --api_data dashstats,cluster_status,capacity_by_path
python3 app.py --op aggregate_data

# Pull IOPS data from the qumulo cluster once per minute
(crontab -l ; echo "* * * * * . $QPROFILEPATH; cd $PWD; python3 app.py --op api_pull --api_data iops_by_path >> cron-api-data-1-minute.log 2>&1")| crontab -
# Pull capacity and activity data from the qumulo every six hours
(crontab -l ; echo "1 */6 * * * . $QPROFILEPATH; cd $PWD; python3 app.py --op api_pull --api_data dashstats,cluster_status,capacity_by_path >> cron-api-data-6-hour.log 2>&1")| crontab -
